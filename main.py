...
players_sprites.add(player) # ajout du joueur 1 dans le groupe des sprites
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key==pygame.K_SPACE:
                ...
                bullets_P1.add(Ball(player.rect.x, player.rect.y, mouse_x, mouse_y))
    ...
    player.changeMoveSet([keys[pygame.K_q],keys[pygame.K_d],keys[pygame.K_z],keys[pygame.K_s]])
    ...
    hit_On_P1=pygame.sprite.spritecollide(player,bullets_P2,True)
    if hit_On_P1:
